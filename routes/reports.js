...
router.get("/kmom03", (req, res) => {
    db.each("SELECT content FROM reports WHERE kmom=3", (err, row) => {
        if (err) {
            return res.status(500).json({
                errors: {
                    status: 500,
...
router.get("/kmom05", (req, res) => {
    db.each("SELECT content FROM reports WHERE kmom=5", (err, row) => {
        if (err) {
            return res.status(500).json({
                errors: {
                    status: 500,
...
router.get("/kmom06", (req, res) => {
    db.each("SELECT content FROM reports WHERE kmom=6", (err, row) => {
        if (err) {
            return res.status(500).json({
                errors: {
                    status: 500,
